...
simulationRouter.get("/", async (req, res) => {
  try {
    const simulations = await prisma.simulationParams.findMany({
      orderBy: {
        createdAt: "desc",
      },
      include: {
...
simulationRouter.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const simulation = await prisma.simulationParams.findUnique({
      where: {
        id: parseInt(id),
      },
      include: {
...
simulationRouter.post("/", async (req, res) => {
  try {
    ...
    const simulation = await prisma.simulationParams.create({
      data: {
 ...
 *         description: Server error
 */
simulationRouter.post("/:id/run", async (req, res) => {
  try {
    const { id } = req.params;
...
simulationRouter.delete("/:id", async (req, res) => {
  try {
    ...
    await prisma.simulationParams.delete({
      where: {
        id: parseInt(id),
      },
    });

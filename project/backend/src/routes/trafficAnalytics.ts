...
export const trafficAnalyticsRouter = express.Router();
 ...
 */
trafficAnalyticsRouter.get("/snapshots", async (req, res) => {
  try {
    ...
    if (snapshots.length === 0) {
      ...
      const syntheticSnapshots = generateSyntheticSnapshots(currentDate, limit ? parseInt(limit as string) : 24);
 ...
 *         description: Server error
 */
trafficAnalyticsRouter.post("/snapshots", async (req, res) => {
  try {
    ...
    const snapshot = await prisma.trafficSnapshot.create({
      data: {
 ...
 *         description: Server error
 */
trafficAnalyticsRouter.get("/daily", async (req, res) => {
  try {
    ...
    const dailyData = await prisma.dayData.findMany({
      where: {
        date: {
          gte: startDate,
          lte: endDate,
 ...
 *         description: Server error
 */
trafficAnalyticsRouter.post("/daily", async (req, res) => {
  try {
    ...
    const existingData = await prisma.dayData.findUnique({
      where: {
        date: inputDate,
      },
    });
    ...
    if (existingData) {
      ...
      const updatedData = await prisma.dayData.update({
        where: {
          id: existingData.id,
        },
        data: {
      ...
      // Create new data
      const newData = await prisma.dayData.create({
        data: {
          date: inputDate,
          totalVehicles,
          peakCongestion,
...
function generateSyntheticSnapshots(date: Date, count: number) {
  const snapshots = [];
...
function generateSyntheticDailyData(startDate: Date, days: number) {
  const dailyData = [];
...
export default trafficAnalyticsRouter; 
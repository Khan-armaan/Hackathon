...
exports.adjustWeightsForSimulation = adjustWeightsForSimulation;
exports.dijkstra = dijkstra;
exports.aStar = aStar;
// Build a graph from traffic data
function buildGraph(trafficData) {
    const nodes = new Map();
    ...
    trafficData.forEach(road => {
        ...
        const distance = Math.sqrt(Math.pow(road.endX - road.startX, 2) +
...
function adjustWeightsForSimulation(graph, trafficData, simulationParams, events) {
    ...
    graph.nodes.forEach((node, id) => {
        newGraph.nodes.set(id, Object.assign(Object.assign({}, node), { connections: [...node.connections.map(conn => (Object.assign({}, conn)))] }));
    });
    // Apply simulation parameter adjustments
    if (simulationParams) {
        // Time of day affects traffic patterns
        const timeMultiplier = getTimeOfDayMultiplier(simulationParams.timeOfDay);
        // Day type affects overall traffic volume
        const dayTypeMultiplier = getDayTypeMultiplier(simulationParams.dayType);
        // Weather affects road speeds
        const weatherMultiplier = getWeatherMultiplier(simulationParams.weatherCondition);
    ...
    if (events && events.length > 0) {
        const activeEvents = events.filter(event => event.status === 'UPCOMING' || event.status === 'ONGOING');
...
function dijkstra(graph, startNodeId, endNodeId) {
    ...
    const roadUsed = new Map(); // key: "fromNode-toNode", value: roadId
...
function heuristic(graph, nodeId, goalId) {
    ...
    if (!node || !goal)
        return Infinity;
    return Math.sqrt(Math.pow(goal.x - node.x, 2) +
        Math.pow(goal.y - node.y, 2));
}
// Build the path from the result of pathfinding algorithms
function buildPath(graph, previous, roadUsed, startNodeId, endNodeId, totalWeight) {
    const path = [];
    const roadPath = [];
    ...
    while (current !== startNodeId) {
        ...
        if (roadId)
            roadPath.unshift(roadId);
        // Add coordinates
        const node = graph.nodes.get(current);
        if (node)
            coordinatePath.unshift({ x: node.x, y: node.y });
        current = prev;
    }
    // Add the start node
    ...
    if (startNode)
        coordinatePath.unshift({ x: startNode.x, y: startNode.y });
    return {
        path,
        roadPath,
        totalWeight,
...
function getTimeOfDayMultiplier(timeOfDay) {
    switch (timeOfDay) {
...
function getDayTypeMultiplier(dayType) {
    switch (dayType) {
...
function getEventImpactMultiplier(impactLevel) {
    switch (impactLevel) {
        case 'LOW': return 1.2;
